...
public class T07AddWhere extends TesterBase
{
	...
	try{
	    Boolean ret = (Boolean) addPicturePW_M.invoke(alb,Two180,0);
	    successes = successes && ret.booleanValue();

	    successes = successes && 
		((Boolean) addPicturePW_M.invoke(alb,Four160,1)).booleanValue();
	    successes = successes && 
		((Boolean) addPicturePW_M.invoke(alb,Three100,1)).booleanValue();
	    successes = successes && 
	    	((Boolean) addPicturePW_M.invoke(alb,One150,0)).booleanValue();
	...
	try{
	    failure = ((Boolean) addPicturePW_M.invoke(alb,One150,0)).booleanValue();
	    explore_M.invoke(alb);
	}
	catch( Exception e )
	    {
