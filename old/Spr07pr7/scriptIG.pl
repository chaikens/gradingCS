...
$Course = "CSI310";   #Course name/nr for mail subject line.
...
my(@UnwantedFiles)  = ("*.o","*.s","$exeName","core","my_executable","a.out");
...
$RequestOneDirBeSubmitted = 1;
$OneDirSubmissionPoints = 5; 
...
$BuildScriptPoints = 10;      # Self-documenting. 
$BuildScriptRequested = 1;
...
$valueInteractiveTests = 70.0;
#Spr06 Proj2: 18 regular text cases*4 + 2 honors cases*4
...
# On time Project Deadline  05/01/07 23:00 (11:00PM)
$DUEhour = 23;
$DUEmin  = 00;
$DUEmon  = 4;         # Months past January (0-11) so February = 1
$DUEmday = 01;
$DUEyear = 107;        # Years past 1900
$DUEtime = SetDueTime($DUEyear,$DUEmon,$DUEmday,$DUEhour,$DUEmin);  # Secs. past epoch

# Early bird Deadline  12/7/00 21:00 (9:00PM)
$EARLYhour = 21;
$EARLYmin  = 01;
$EARLYmon  = 11;         # Months past January (0-11) so February = 1
$EARLYmday = 7;
$EARLYyear = 100;        # Years past 1900
...
@PartsIGList = ("find-path", "find-dists");
@PartsIGexeName = ("find-path", "find-dists");
@PartsIGTestCaseDir =
    ("$TestCaseDir/find-path", "$TestCaseDir/find-dists"  );
...
$GradeReportsDir = "$ProjGradingDir/Reports$TAUserName";
...
$GradeSummaryFile = "$ProjGradingDir/summary$TAUserName";
...
$NoReportFile = "$ProjGradingDir/no_report$TAUserName";
...
$TestCWD = "$TestCompileDir/_TESTING_";
...
{
    chdir $TestCompileDir;
    #CSI310 requires submission of a directory.
...
sub tryBuildNoMakefile
{
    chdir ( $TestCompileDir );    
    my($success)= 0;
    my( $compiler );
#    @c_files = glob("$TestCompileDir/*.c");
    @cpp_index = ("$TestCompileDir/*.cc","$TestCompileDir/*.cpp","$TestCompileDir/*.C");
    my($index);
    $index = 0;
    for ($index = 0; $index < 3; $index++) {
	@cpp_files = glob("$cpp_index[$index]");
	$index += 1;
    }

# Hacked in fix to check for cpp and C as well as .cc files


# please fix-perl reports undefined var when @cpp_files is the empty array,
# which is not an error situation. FIXED
# Previous flawed test condition    if ($cpp_files[0] ne "")

    if ( defined($cpp_files[0]) )
    {
	$compiler = "g++";
    }
    else
    ...
    #
    # Hack to create a g++ command that handles general set of 
    # .cpp, .C and .cc files
    #
    $source_files = `ls *.cpp *.cc *.C *.c`;
    $source_files =~ s/\n/ /g;  #replace all newlines with spaces
    $compile_command = "$compiler -g $source_files  -o $exeName > $CompileMessageFile 2>&1";
    print "Compiling with $compile_command\n";
#    print GROUT  "  Compiling with compile command $compile_command\n";
    $compile_return = system($compile_command);
...
# ( student output file name in $TestCWD, 
