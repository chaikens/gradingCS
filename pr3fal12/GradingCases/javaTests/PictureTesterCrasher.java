/**
 * PictureTesterCrasher.java
 * tests the box manipulation methods on an illegal box range
 * and test the return value, to award extra credit.
 * I found I had not assigned that, so we are not doing that
 * in production.
 */
...
new Picture("/home/faculty1/sdc/public_html/CSI201/Fal12/Proj/Proj3/beach.jpg");
	...
	out.println("Now, testing the manip with BAD box parameters");

	Object Ret = maniP(m,p, -1, 3, 4, 3, 1.0);
	out.println("manip function returned: " + Ret);
	int extra = 0;
	if(Boolean.FALSE.equals(Ret))
	    {
		extra = 3;
		out.println("Extra credit of 3 for detecting bad parameters!");
	    }
        return scin+extra;
    }
