...
$filesWeBuild = 
    ".class file(s) from compiling your Java source files";
#"Turtle.class";
#"Picture.class";
...
#In Project 2, we subtract 20 points per deadline
#Commented out to stop Perl warning.
#$DUEmaxdayslate = 7.0;

#Inverse Penalty Rate.  If this is say 12, penalty is 1/12 or 8.333% per day.
#We don't use this for Project 1.  
#commented out to stop Perl's warning
...
$publicTestCaseBaseURL = 
    "http://www.cs.albany.edu/~sdc/CSI201/Fal12/Proj/Proj1/";
...
@PartsIGList =  ("run-it");#("whole-vignette", "rect-vignette");
@PartsIGexeName = ("$TestCaseDir/run-it/run-main.pl" );
   #("$TestCaseDir/whole-vignette/WholeUnitTest.sh", 
                         #"$TestCaseDir/rect-vignette/RectUnitTest.sh");
@PartsIGTestCaseDir = ("$TestCaseDir/run-it" );
    #("$TestCaseDir/whole-vignette", "$TestCaseDir/rect-vignette"  );
@PartsIGPenaltyPolicyFile = 
    ("$TestCaseDir/penalties"   
    );

@PartsIGPoints = (89); #(80, 20);
...
$TestCompileDir, 
   ...
   then fg.
Type y if you succeeded in compiling it and n if not.\n";
...
sub tryBuildWithCommand
{
    ...
    #FOR THIS PROJECT, WE JUST TRY TO RUN THE JAVA APPLICATION.
...
#	$ScorePenalized = int(($ScorePenalized*10)+0.5)/10.0;	
	$ScorePenalized = $Score - $pointsSubtracted;
	if($ScorePenalized < 0.0){$ScorePenalized = 0;}
